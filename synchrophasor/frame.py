    ...
    **Raises:**
    ...
    def set_id_code(self, id_code):
        ...
        **Raises:**
        ...
        if not 1 <= int(id_code) <= 65534:
            raise FrameError("ID CODE out of range. 1 <= ID_CODE <= 65534")
        else:
            print(id_code, " type of ID code is ", type(id_code))
            self._pmu_id_code = int(id_code)
...
class ConfigFrame1(CommonFrame):
    ...
    **Raises:**
    ...
    def split_cfg(self):
        ...
        pmu_id = int(self.get_id_code())

        cfg = []
        station_names = self.get_station_name()

        for i in range(self.get_num_pmu()):
            cfg.append(ConfigFrame2(pmu_id + i, 1000000, 1, self.get_station_name()[i],
                                    pmu_id+i, self.get_data_format()[i], self.get_phasor_num()[i],
